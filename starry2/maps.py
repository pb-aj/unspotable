...
class MapBase(object):
    ...
    def __setitem__(self, idx, val):
        ...
        if isinstance(idx, integers) or isinstance(idx, slice):
            ...
            #Original starry does not allow for Y00 to be changed
            #Removed this feature to allow for better manipulation of final maps
            """
            if 0 in inds:
    ...
    def _updatefig_all(
        ...
        flux_image = extra_lines[0][1]
        flux_data = extra_lines[0][0]

        rv_image = extra_lines[1][1]
        rv_data = extra_lines[1][0]

        new_flux_data = np.concatenate((flux_data[i:],flux_data[:i]))
        new_rv_data = np.concatenate((rv_data[i:],rv_data[:i]))
    ...
    def _updatefig_line(
        ...
        new_limb_data = np.concatenate((limb_data[i:],limb_data[:i]))

        limb_image[0].set_ydata(new_limb_data)
    ...
    def show(self, **kwargs):
        ...
        extra_image = kwargs.pop("extra_image",None) #Adding option to animate two images together
        """extra_image is a list containing the img, image, and lonlines values 
                    ...
                    if "zmqshell" in str(type(get_ipython())):
                        ...
                        with matplotlib.rc_context(
                            ...
                            if show_image:
                                ...
                                return [img,image,lonlines,latlines]
                    else:
                        raise NameError("")
                except NameError:
                    if show_image:
